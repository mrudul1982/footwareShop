...
if(isset($_POST['add_product'])){
   ...
   $selected_category_name = mysqli_real_escape_string($conn, $_POST['category']);

   // Retrieve the category ID based on the selected category name
   $select_category_id = mysqli_query($conn, "SELECT id FROM `category` WHERE catname = '$selected_category_name'");
   ...
   if(mysqli_num_rows($select_product_name) > 0){
      ...
      $insert_product = mysqli_query($conn, "INSERT INTO `products`(name, details, price, image, cate_id) VALUES('$name', '$details', '$price', '$image', '$category_id')") or die('query failed');
      ...
      <select name="category" class="box" required>
         ...
         if(mysqli_num_rows($select_categories) > 0){
            while($fetch_categories = mysqli_fetch_assoc($select_categories)){
               echo "<option value='".$fetch_categories['catname']."'>".$fetch_categories['catname']."</option>";
            }
         }
         ?>
      </select>
